...
- `thought` is capped at 200 characters when persisted. If a model returns a longer thought, it is silently truncated. This prevents unbounded prompt growth since thoughts are injected into subsequent turn directives.
...
5. Utterance cleaning:
   ...
   - Maximum length: > 200 characters → truncate at last sentence boundary within cap. If no sentence boundary exists, treat as silence. This guards against malformed fallback output flooding the transcript.
